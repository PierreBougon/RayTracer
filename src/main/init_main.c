...
** Last update Fri May 20 14:36:39 2016 benjamin duhieu
*/
...
{
  if ((ftabs->inters_ftab = bunny_malloc(sizeof(ftabs->inters_ftab) * 8)) == NULL)
    return (my_puterr("Could not malloc ftabs"));
  if ((ftabs->shadow_ftab = bunny_malloc(sizeof(ftabs->shadow_ftab) * 5)) == NULL)
    return (my_puterr("Could not malloc ftabs"));
  if ((ftabs->hit_ftab = bunny_malloc(sizeof(ftabs->hit_ftab) * 8)) == NULL)
    return (my_puterr("Could not malloc ftabs"));
